...
このリポジトリは **NixOS が既にインストールされている環境** での使用を想定しています。

### NixOS のインストールがまだの場合

1. [NixOS 公式サイト](https://nixos.org/download.html) から ISO をダウンロード
2. USB メモリに書き込んでブート
3. [公式インストールガイド](https://nixos.org/manual/nixos/stable/#sec-installation) に従って基本インストール
   - パーティション設定
   - 最小限の `configuration.nix` で OK
4. 再起動後、下記の「クイックスタート」に進む

> 💡 **ヒント**: 最小限の設定でインストールし、その後このリポジトリの設定を適用するのがおすすめです
